...
export default function Admin() {
  ...
  const [managingBag, setManagingBag] = useState(null)
  const [bagParticipants, setBagParticipants] = useState([])
  ...
  useEffect(() => {
    if (!managingBag) return
    supabase.from('participants').select('*').eq('bag_id', managingBag.id).order('created_at')
      .then(({ data }) => { if (data) setBagParticipants(data) })
  }, [managingBag])
  ...
  async function removeParticipant(id) {
    ...
    setBagParticipants(p => p.filter(x => x.id !== id))
  }
  ...
  async function handlePhotoUpload(e) {
    ...
    const { error } = await supabase.storage.from('bag-photos').upload(filename, file, { upsert: true })
    if (error) { alert('upload failed: ' + error.message); setUploadingPhoto(false); return }
    const { data: urlData } = supabase.storage.from('bag-photos').getPublicUrl(filename)
    ...
    <div style={{ minHeight: '100vh', background: 'var(--surface)', padding: '2rem', maxWidth: '900px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', borderBottom: '1px solid var(--tile-border)', paddingBottom: '1rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <a href="/" style={{ fontFamily: 'var(--font-display)', fontSize: '11px', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--muted)', border: '1px solid var(--tile-border)', padding: '8px 16px', textDecoration: 'none', transition: 'all 0.2s' }}
        ...
        {bags.map(bag => (
          <div key={bag.id} style={{ background: 'var(--tile)', border: '1px solid var(--tile-border)', padding: '1rem', marginBottom: '8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexShrink: 0, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
              ...
              <button onClick={() => setManagingBag(bag)} style={{ ...btnStyle, background: 'transparent', color: 'var(--muted)', borderColor: 'var(--tile-border)' }}>participants</button>
            ...
            <Field label="Photo">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {editingBag.photo_url && (
                  <img src={editingBag.photo_url} alt="preview" style={{ width: '100%', maxHeight: '200px', objectFit: 'cover', opacity: 0.85, filter: 'saturate(0.7) contrast(1.1)' }} />
                )}
                <label style={{ background: 'transparent', border: '1px solid var(--tile-border)', color: 'var(--muted)', fontFamily: 'var(--font-display)', fontSize: '11px', letterSpacing: '0.2em', textTransform: 'uppercase', padding: '10px 16px', cursor: 'pointer', textAlign: 'center', display: 'block' }}>
                  {uploadingPhoto ? 'uploading...' : editingBag.photo_url ? 'replace photo' : 'upload photo'}
                  <input type="file" accept="image/*" style={{ display: 'none' }} onChange={handlePhotoUpload} />
                </label>
                {editingBag.photo_url && (
                  <input style={{ ...inputStyle, fontSize: '10px', color: 'var(--muted)' }} value={editingBag.photo_url} onChange={e => setEditingBag(b => ({ ...b, photo_url: e.target.value }))} placeholder="or paste a URL" />
                )}
              </div>
            </Field>
      ...
      {managingBag && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: '1rem' }}>
          <div style={{ background: 'var(--tile)', border: '1px solid var(--tile-border)', padding: '2rem', width: '100%', maxWidth: '480px', maxHeight: '90vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '12px', letterSpacing: '0.3em', textTransform: 'uppercase', color: 'var(--muted)' }}>participants — {managingBag.name}</div>
              <button onClick={() => setManagingBag(null)} style={{ ...btnStyle, background: 'transparent', color: 'var(--muted)', borderColor: 'var(--tile-border)', padding: '6px 12px' }}>✕</button>
            </div>
            {bagParticipants.length === 0 ? (
              <div style={{ color: 'var(--muted)', fontFamily: 'var(--font-display)', fontSize: '12px', letterSpacing: '0.1em' }}>no participants yet.</div>
            ) : (
              bagParticipants.map(p => (
                <div key={p.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--flap-bg)', border: '1px solid var(--tile-border)', padding: '10px 12px' }}>
                  <div>
                    <div style={{ fontFamily: 'var(--font-display)', fontSize: '13px', color: 'var(--off-white)', letterSpacing: '0.05em' }}>{p.name}</div>
                    <div style={{ fontFamily: 'var(--font-display)', fontSize: '10px', color: 'var(--muted)', letterSpacing: '0.1em', marginTop: '2px' }}>{new Date(p.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</div>
                  </div>
                  <button onClick={() => removeParticipant(p.id)} style={{ ...btnStyle, background: 'transparent', color: '#e05050', borderColor: '#e05050', padding: '6px 12px', fontSize: '10px' }}>remove</button>
                </div>
              ))
            )}
          </div>
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', marginRight: '4px' }}>
                <button
                  ...
                  disabled={i <= 1}
                  style={{ ...btnStyle, padding: '2px 8px', fontSize: '10px', opacity: i <= 1 ? 0.2 : 1, background: 'transparent', color: 'var(--muted)', borderColor: 'var(--tile-border)' }}
                >▲</button>
                <button
                  onClick={() => {
                  ...
                  style={{ ...btnStyle, padding: '2px 8px', fontSize: '10px', opacity: (i === 0 || i === logoWords.length - 1) ? 0.2 : 1, background: 'transparent', color: 'var(--muted)', borderColor: 'var(--tile-border)' }}
                >▼</button>
              </div>
              <div style={{ flex: 1, background: 'var(--flap-bg)', border: '1px solid var(--tile-border)', padding: '8px 10px', fontFamily: 'var(--font-display)', fontSize: '13px', color: i === 0 ? 'var(--muted)' : 'var(--off-white)', letterSpacing: '0.1em' }}>
                {word}{i === 0 ? ' (locked)' : ''}
              </div>
              {i > 0 && (
